...
class Camera2 extends CameraViewImpl {
    ...
    Camera2(Callback callback, PreviewImpl preview, final Context context) {
        ...
        mPreview.setOnTouchLinster(new PreviewImpl.OnTouchLinster() {
            @Override
            public void onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case MotionEvent.ACTION_DOWN:
                        Log.e("touch","X坐标：" + event.getX()+",Y坐标："+event.getY());
//                        triggerFocusArea(event.getX(),event.getY());

                        int areaX = (int) (event.getX() / mPreview.getWidth() * 2000) - 1000; // 获取映射区域的X坐标
                        int areaY = (int) (event.getY() / mPreview.getWidth() * 2000) - 1000; // 获取映射区域的Y坐标
                        break;
                    default:
                        break;
                }
    ...
    private void triggerFocusArea(float x, float y) {
        try {
//            Rect cropRegion = AutoFocusHelper.cropRegionForZoom(mCameraCharacteristics, 1.0f);
//            mAERegions = AutoFocusHelper.aeRegionsForNormalizedCoord(x, y, cropRegion, 0);
//            mAFRegions = AutoFocusHelper.afRegionsForNormalizedCoord(x, y, cropRegion, 0);
...
//            mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER, CameraMetadata.CONTROL_AE_PRECAPTURE_TRIGGER_START);
            ...
            android.util.Size size = mCameraCharacteristics.get(CameraCharacteristics.SENSOR_INFO_PIXEL_ARRAY_SIZE);
