...
import com.google.android.cameraview.demo.camera.exif.ExifInterface;
...
public class FileSaver {
    ...
    public void saveFile(int width, int height, int orientation, final byte[] data, String tag,
                         int saveType) {
    ...
    private void saveYuvFile(final ImageInfo info) {
        ...
        final Bitmap thumbnail = BitmapFactory.decodeResource(
                mContext.getResources(), R.drawable.yuv_file);
        if (mListener != null) {
            mHandler.post(new Runnable() {
                @Override
                public void run() {
